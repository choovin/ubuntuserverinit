...
################################################################################
# CentOS/RHEL/OpenCloud/Alibaba Cloud Server Initial Setup Script
#
...
OS_TYPE="centos"
LOG_FILE="$HOME/centos-setup-$(date +%Y%m%d-%H%M%S).log"
...
detect_os() {
    if [ -f /etc/redhat-release ]; then
        if grep -q "Alibaba" /etc/redhat-release; then
            OS_TYPE="alibaba"
...
install_system_updates() {
    ...
    if ask_yn "Update system packages?" "y"; then
        ...
        sudo dnf install -y curl wget vim git tar gzip ca-certificates
...
install_docker() {
    ...
    if ask_yn "Install Docker CE?" "y"; then
        ...
        sudo dnf remove -y docker docker-client docker-client-latest docker-common docker-latest docker-latest-logrotate docker-logrotate docker-engine docker-compose 2>/dev/null || true
        ...
        log_info "Installing Docker CE and dependencies..."
...
install_zoxide() {
    ...
    if ask zoxide (smart_yn "Install cd command)?" "y"; then
...
install_neovim() {
    ...
    if ask_yn "Install Neovim?" "y"; then
        ...
        if ! sudo curl -fL "$NVIM_URL" -o /opt/nvim/nvim.appimage; then
            log_info "Installing Neovim from EPEL..."
            sudo dnf install -y epel-release || true
            sudo dnf install -y neovim python3-neovim || true
            if command_exists nvim; then
...
install_poetry() {
    log_header "Poetry Installation (Python Package Manager)"
...
install_ripgrep_fd() {
    ...
    if ask_yn "Install ripgrep & fd?" "y"; then
        ...
        if ! command_exists fd; then
            ...
            sudo dnf install -y fd || sudo dnf install -y fdfind || true
        fi
...
main() {
    log_header "CentOS/RHEL/OpenCloud/Alibaba Server Setup"
